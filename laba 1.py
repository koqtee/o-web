...
x_nodes = np.arange(0.0, 2.1, 0.5)  # от 0.0 до 2.0 с шагом 0.5
y_nodes = np.arange(0.0, 2.1, 0.5)  # от 0.0 до 2.0 с шагом 0.5
...
for i in range(len(x_nodes)):
    for j in range(len(y_nodes)):
        if point_in_triangle(X_nodes[j, i], Y_nodes[j, i], A, B, C):
            plt.scatter(X_nodes[j, i], Y_nodes[j, i], color='gray', s=30, marker='o', alpha=0.7)
...
# График 6: Решение с шагом 0.25 с узлами сетки
plt.subplot(2, 3, 6)
phi2_masked = np.where(triangle_mask2, phi2, np.nan)
contour = plt.contourf(X2, Y2, phi2_masked, levels=20, cmap='viridis')
plt.colorbar(contour, label='φ(x, y)')

# Узлы сетки с шагом 0.25 и началом в 0.0
x_nodes = np.arange(0.0, 2.1, 0.25)  # от 0.0 до 2.0 с шагом 0.25
y_nodes = np.arange(0.0, 2.1, 0.25)  # от 0.0 до 2.0 с шагом 0.25
X_nodes, Y_nodes = np.meshgrid(x_nodes, y_nodes)
...
for i in range(len(x_nodes)):
    for j in range(len(y_nodes)):
        if point_in_triangle(X_nodes[j, i], Y_nodes[j, i], A, B, C):
            plt.scatter(X_nodes[j, i], Y_nodes[j, i], color='black', s=15, marker='o', alpha=0.6)

plt.contour(X2, Y2, phi2_masked, levels=10, colors='black', alpha=0.3)
plt.plot(vertices[:, 0], vertices[:, 1], 'b-', linewidth=1)
plt.plot([A[0], B[0]], [A[1], B[1]], 'r-', linewidth=2)
plt.title('Решение φ(x,y) для h=0.25\nс узлами сетки', fontsize=12)
plt.xlabel('x')
plt.ylabel('y')
plt.axis('equal')
...
print(f"     Угол A = 120°")
print(f"     Площадь: {0.5 * 2 * 2 * sin(radians(120)):.4f}")import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
from math import cos, sin, radians, sqrt
